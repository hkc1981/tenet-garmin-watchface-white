            ...
            for x in range(width):
                ...
                if a > 0:
                    ...
                    if gray > 220:
                        ...
                        # 【保持輪廓線條清晰紮實】
                        # 如果像素原本是暗部（gray < 130，代表娃娃的五官、輪廓外框線與細節線），
                        # 我們強制將其 Alpha 設為完全不透明（255），確保外框在手錶黑底上紮實顯現，絕不模糊斷裂！
                        if gray < 130:
                            new_alpha = 255
                        else:
                            # 偏亮區域（gray >= 130）進行平滑漸變透明映射，保留細節陰影
                            new_alpha = int((220 - gray) * (255.0 / (220 - 130)))
                            if new_alpha < 10:
                                new_alpha = 0
                        
                        # 統一染色為淺灰色 (170, 170, 170) 並套用輪廓保護 Alpha
